...
function! ruby_refactoring#get_input(message, error_message)
...
function! ruby_refactoring#get_visual_selection(...)
  try
    let a_save = @a
    if a:0 >= 1 && a:1 == 1
      normal! gv"ad
...
function! ruby_refactoring#get_range_for_block(pattern_start, flags)
...
function! ruby_refactoring#gsub_all_in_range(start_line, end_line, pattern, replace)
...
function! ruby_refactoring#dedupe_list(target)
...
function! ruby_refactoring#cut_visual_selection()
  return ruby_refactoring#get_visual_selection(1)
endfunction
